...
export default function Sidebar() {
    ...
    return (
        <aside className={`${isCollapsed ? 'w-20 px-3' : 'w-72 p-6'} h-screen sticky top-0 bg-neutral-900/95 border-r border-white/5 flex flex-col transition-all duration-300 ease-in-out backdrop-blur-3xl z-[100]`}>
                ...
                <div className={`flex items-center gap-4 group ${isCollapsed ? 'justify-center' : ''}`}>
                    ...
                    {!isCollapsed && (
                        <div className="animate-in fade-in slide-in-from-left-4 duration-300">
                            ...
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-neutral-500 mt-0.5">Console v2.6</p>
                        </div>
                    )}
                </div>
            ...
            <nav className="flex-1 space-y-2">
                {!isCollapsed && (
                    <div className="px-3 mb-2 animate-in fade-in duration-300">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-600">Navigation</span>
                    </div>
                )}
                {menuItems.map((item) => (
                    <SidebarItem
            ...
            {isCollapsed && (
                <button
                    ...
                    className="absolute bottom-4 right-[-12px] bg-neutral-800 border border-neutral-700 rounded-full p-1 text-white shadow-lg hover:scale-110 transition-transform"
                >
